...
# Google API key Details    
api_key = 'AIzaSyC3VwOSfjrOGep91u9P7jlL-zkSbuuR0o8'
...
# MYSQL Connection
engine = create_engine('mysql+mysqlconnector://root:admin123@localhost/youtube_data')
...
# Getting channel,video,playlist,comment data from youtube
...
def channel_list():
    ...
    channel_list.reverse()  # Reverse the channel list easy to upload the data into sql
    return channel_list
channel_list()


def channel_video_comment(channel_id):
...
def extract_comment(channel_name_to_find):
    ...
    for result in finding:
        ...
        for video_id, video_data in video_dict.items():
            ...
            comments = video_data.get('Comments', {})  # if no comments
...
# Creating Table and Store Data in SQL Database
...
# Fuction to Migrate Mongo data into SQL

def Mongo_to_SQL(channel_name_to_find):
...
def question_6():
    mycursor.execute("SELECT channel_name, video_name, like_count FROM video_data ORDER BY like_count DESC;")
    data = mycursor.fetchall()
    data_df = pd.DataFrame(data, columns=[ 'CHANNEL_NAME', 'VIDEO_NAME', 'LIKE_COUNT']).reset_index(drop=True)
    data_df.index += 1
    return data_df
